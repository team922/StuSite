function StartList() {
    ...
    if (getQueryString("type") == "notice") {
        //alert("notice");
        gettopicnotice();
        GetNoticeList(pagesize, pageindex);
    }
    else {
        //alert("news");
        gettopicnews();
        GetNewsList(pagesize, pageindex);
    }
}
...
function GetList(pageindex) {
    ...
    var pagesize = getQueryString("pagesize");
    if (type == "notice") {
        window.location.href = "/Admin/NoticeManage2/?type=" + type + "&pagesize=" + pagesize + "&pageindex=" + pageindex;
    }
    else {
        window.location.href = "/Admin/NewsManage2/?type=" + type + "&pagesize=" + pagesize + "&pageindex=" + pageindex;
    }
}
...
function confirmchack(o) {
    //alert("1111");
    if (window.confirm('该操作不可撤销，是否继续？')) {
        //alert("确认");
        return true;
    } else {
        //alert("取消");
...
function addtopicnotice(id, title, belong, date) {
    var topic = "<img src='/Images/main_istop.png' /><a id='" + id + "' class='notice_title' href='###' onclick='opendetail(this);' title='" + title + "'>" + title + "</a><a class='list_del' href='javascript:notice_delete(" + id + ")'>删除</a><a class='list_istopic' href='javascript:notice_removetopic(" + id + ")'>取消置顶</a><div class='list_date'>" + date + "</div><div class='list_belong' title='" + belong + "'>" + belong + "</div>";
    //alert(topic);
    $("#list_topic").append(topic);
}

function addnormalnotice(id, title, belong, date) {
    var list = "<div class='list_normal'><img src='/Images/main_right.png' /><a id='" + id + "' class='notice_title' href='###' onclick='opendetail(this);' title='" + title + "'>" + title + "</a><a class='list_del' href='javascript:notice_delete(" + id + ")'>删除</a><a class='list_istopic' href='javascript:notice_settopic(" + id + ")'>设置置顶</a><div class='list_date'>" + date + "</div><div class='list_belong' title='" + belong + "'>" + belong + "</div></div>";
    //alert(list);
    $("#list_normal_all").append(list);
}

function addtopicnews(id, title, date) {
    var topic = "<img src='/Images/main_istop.png' /><a id='" + id + "' class='news_title' href='###' onclick='opendetail(this);' title='" + title + "'>" + title + "</a><a class='list_del' href='javascript:news_delete(" + id + ")'>删除</a><a class='list_istopic' href='javascript:news_removetopic(" + id + ")'>取消置顶</a><div class='list_date'>" + date + "</div>";
    $("#list_topic").append(topic);
}

function addnormalnews(id, title, date) {
    var list = "<div class='list_normal'><img src='/Images/main_right.png' /><a id='" + id + "' class='news_title' href='###' onclick='opendetail(this);' title='" + title + "'>" + title + "</a><a class='list_del' href='javascript:news_delete('" + id + ")'>删除</a><a class='list_istopic' href='javascript:news_settopic(" + id + ")'>设置置顶</a><div class='list_date'>" + date + "</div></div>";
    $("#list_normal_all").append(list);
}
...
function make_page_list(pagecount, pageindex) {
    ...
    if (pagecount > 1) {
        var html = '<table id="page_list_table" cellpadding="0" width="600px" cellspacing="0" height="40" align="center"> <tr>';
